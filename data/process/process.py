...
img_dir = root + 'process/img/'
xml_dir = root+'process/xml/'
txt_dir = root+'process/txt/'


out_dir = root+'process/out/'
vis_dir = root+'process/vis/'

detect_dir = root + "detect_new/"
train_dir = root+"train_new/"
test_dir = root+"test_new/"
...
def processimgtest(imgpath,txtpath):
    img = cv2.imread(imgpath, cv2.IMREAD_COLOR)

    cv2.imwrite('data/process/imgtest/origin.jpg', img)
    ...
    cv2.imwrite('data/process/imgtest/size.jpg', kimg)
    ...
    cv2.imwrite('data/process/imgtest/label.jpg', tempimg)
    ...
    cv2.imwrite('data/process/imgtest/rotate.jpg', kimg)
    ...
    cv2.imwrite('data/process/imgtest/move.jpg', kimg)

    kimg = gasuss_noise(kimg, 0, 0.003)
    cv2.imwrite('data/process/imgtest/noise.jpg', kimg)

    kimg,pad = pad_to_square(kimg,0)
    cv2.imwrite('data/process/imgtest/pad.jpg', kimg)

    cv2.resize(kimg,(416,416),kimg,interpolation=cv2.INTER_LINEAR)
    cv2.imwrite('data/process/imgtest/resize.jpg', kimg)


def creat(img_dir, txt_dir, targetpath, times, islabel, isgaosi):
    # 获得所有图片
    ImgFiles = Get_JpgTxtfiles(img_dir, '.jpg')
    ...
    for i in range(len(ImgFiles)):
        ...
        for k in range(times):
            ...
            img_neme = (os.path.splitext(ImgFiles[i])[
                        0]+'_'+str(k)).zfill(10) + '.jpg'
            txt_name = (os.path.splitext(ImgFiles[i])[
                        0]+'_'+str(k)).zfill(10) + '.txt'
...
if __name__ == '__main__':
    ...
    creat(img_dir, txt_dir, detect_dir, 3, islabel=True, isgaosi=False)

    # shutil.rmtree(vis_dir)
    # os.makedirs(vis_dir)
    # visualize(val_dir + str(1)+'/',
    #           val_dir + str(1)+'/', vis_dir)
    # processimgtest('data/process/img/6.jpg', 'data/process/txt/6.txt')
