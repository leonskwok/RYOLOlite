...
if __name__ == "__main__":
    ...
    # if backbone_type=="ghostnet":
    #     pthfile = torch.load("weights/ghost_kun.pth")
    #     # pretrained_dict = {"backbone."+k: v for k, v in pthfile.items() if "backbone."+ k in model_dict}
    #     pretrained_dict = {k: v for i, (k, v) in enumerate(
    #         pthfile.items()) if i < 502}
    # else:
    #     # 第552項開始為yololayer，訓練時不需要用到
    #     # pretrained_dict = {k: v for k, v in pretrained_dict.items() if np.shape(model_dict[k]) == np.shape(v)}
    #     pthfile = torch.load("weights/yolov4_kun.pth")
    #     pretrained_dict = {k: v for i, (k, v) in enumerate(pthfile.items()) if i < 552}
    ...
    train_dataset, train_dataloader = split_data(
        args.train_folder, args.img_size, args.batch_size, shuffle=True, augment=True, multiscale=True)

    num_iters_per_epoch = len(train_dataloader)
    scheduler_iters = round(
        args.epochs * len(train_dataloader) / args.subdivisions)
